...
def train_loop(args, labeled_loader, unlabeled_loader, test_loader,
               ...
               t_optimizer, s_optimizer, t_scheduler, s_scheduler, t_scaler, s_scaler, init):
    ...
    for step in range(args.start_step, args.total_steps):
        ...
        with amp.autocast(enabled=args.amp):
            ...
            t_logits, t_feature_logits = teacher_model(t_images)
            t_logits_l = t_logits[0][:batch_size]
            t_feature_logits_l = t_feature_logits[0][:batch_size]
            ...
            if init is False:
                ...
                teacher_model.adaptation_layers = nn.ModuleList(layer_list)
                teacher_model.adaptation_layers.to(args.device)
                optimizer = optim.SGD(teacher_model.parameters(), lr=args.teacher_lr, weight_decay=5e-4, momentum=0.9)
            ...
            s_logits, s_feature_logits = student_model(s_images)
            s_logits_l = s_logits[0][:batch_size]
            s_feature_logits_l = s_feature_logits[0][:batch_size]
            ...
            if init is False:
                ...
                #   the feature distillation in the final version : |Fully Connected Layer(f1) - f2 | ^ 2
                layer_list = []
                teacher_feature_size = s_feature_logits[0].size(1)
                for index in range(1, len(s_feature_logits)):
                    student_feature_size = s_feature_logits[index].size(1)
                    layer_list.append(nn.Linear(student_feature_size, teacher_feature_size))
                student_model.adaptation_layers = nn.ModuleList(layer_list)
                student_model.adaptation_layers.to(args.device)
                optimizer = optim.SGD(student_model.parameters(), lr=args.student_lr, weight_decay=5e-4, momentum=0.9)
                #   define the optimizer here again so it will optimize the net.adaptation_layers
                init = True
            ...
            for index in range(1, len(s_logits)):
                #   logits distillation
                s_loss += CrossEntropy(s_logits[index][:batch_size], s_logits_l.detach(), args.temperature) * args.loss_coefficient
                s_loss += criterion(s_logits[index][:batch_size], targets) * (1 - args.loss_coefficient)
                #   feature distillation
                if index != 1:
                    s_loss += torch.dist(student_model.adaptation_layers[index-1](s_feature_logits[index][:batch_size]), s_feature_logits_l.detach()) * \
        ...
        with amp.autocast(enabled=args.amp):
            with torch.no_grad():
                s_logits_l, s_feature_logits_l = student_model(images_l)
            s_loss_l_new = F.cross_entropy(s_logits_l[0].detach(), targets)
            ...
            for index in range(1, len(t_logits)):
                #   logits distillation
                t_loss += CrossEntropy(t_logits[index][:batch_size], t_logits_l.detach(), args.temperature) * args.loss_coefficient
                t_loss += criterion(t_logits[index][:batch_size], targets) * (1 - args.loss_coefficient)
                #   feature distillation
                if index != 1:
                    t_loss += torch.dist(teacher_model.adaptation_layers[index-1](t_feature_logits[index][:batch_size]), t_feature_logits_l.detach()) * \
                            args.feature_loss_coefficient
                    #   the feature distillation loss will not be applied to the shallowest classifier
...
def evaluate(args, test_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for step, (images, targets) in enumerate(test_iter):
            ...
            targets = targets.to(args.device)
            with amp.autocast(enabled=args.amp):
                outputs, feature_outputs = model(images)
                ensemble = sum(outputs)/len(outputs)
                ensemble.detach_()
                loss = criterion(ensemble, targets)

            acc1, acc5 = accuracy(ensemble, targets, (1, 5))
...
def main():
    ...
    if args.model == "resnet18":
        teacher_model = resnet18(num_classes=args.num_classes)
        student_model = resnet18(num_classes=args.num_classes)
        # student_model = teacher_model = resnet18(num_classes=args.num_classes)
    if args.model == "resnet34":
        teacher_model = resnet34(num_classes=args.num_classes)
        student_model = resnet34(num_classes=args.num_classes)
        # student_model = teacher_model = resnet34(num_classes=args.num_classes)
    if args.model == "resnet50":
        teacher_model = resnet50(num_classes=args.num_classes)
        student_model = resnet50(num_classes=args.num_classes)
        # student_model = teacher_model = resnet50(num_classes=args.num_classes)
    if args.model == "resnet101":
        teacher_model = resnet101(num_classes=args.num_classes)
        student_model = resnet101(num_classes=args.num_classes)
        # student_model = teacher_model = resnet101(num_classes=args.num_classes)
    if args.model == "resnet152":
        teacher_model = resnet152(num_classes=args.num_classes)
        student_model = resnet152(num_classes=args.num_classes)
        # student_model = teacher_model = resnet152(num_classes=args.num_classes)
    if args.model == "wideresnet50":
        teacher_model = wideresnet50(num_classes=args.num_classes)
        student_model = wideresnet50(num_classes=args.num_classes)
        # student_model = teacher_model = wideresnet50(num_classes=args.num_classes)
    if args.model == "resnext50_32x4d":
        teacher_model = resnext50_32x4d(num_classes=args.num_classes)
        student_model = resnext50_32x4d(num_classes=args.num_classes)
        # student_model = teacher_model = resnext50_32x4d(num_classes=args.num_classes)
    if args.model == "resnext101_32x8d":
        teacher_model = resnext101_32x8d(num_classes=args.num_classes)
        student_model = resnext101_32x8d(num_classes=args.num_classes)
        # student_model = teacher_model = resnext101_32x8d(num_classes=args.num_classes)
